...
# Estimate Rolling rank
def rolling_rank(na):
    return rankdata(na)[-1]

# Estimate Rolling product
def rolling_product(na):
    return np.prod(na)
...
def ts_argmin(df, windown=10):
    return df.rolling(window).apply(np.argmin)+1

def ts_rank(df, window= 10):
    return df.rolling(window).apply(rolling_rank)+1
...
def stochastic_oscillator_k(df):
    ...
    :return: pandas.DataFrame
    """
    SOk = pd.Series((df['Close'] - df['Low']) / (df['High'] - df['Low']), name='SCH')
    df = df.join(SOk)
    return df
...
def stochastic_oscillator_d(df, n):
    ...
    """
    SOk = pd.Series((df['Close'] - df['Low']) / (df['High'] - df['Low']), name='SCH')
    SOd = pd.Series(SOk.ewm(span=n, min_periods=n).mean(), name='SCH_' + str(n))
    df = df.join(SOd)
    return df
